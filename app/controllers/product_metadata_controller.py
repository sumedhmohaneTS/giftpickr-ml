...
@product_metadata_controller.route('/product-metadata', methods=["GET"])
...
@product_metadata_controller.route('/product-metadata', methods=["POST"])
def post_product_metadata():
    age = request.args.get('age', type=int)
    gender = request.args.get('gender')
    occasion = request.args.get('occasion')
    relationship = request.args.get('relationship')
